  ...
  <div class="max-w-2xl mx-auto flex flex-col gap-4">
    ...
    <UiCard padded>
      ...
      <UserList
        v-else
        :items="page?.items ?? []"
        :empty-message="$t('profile.following_empty')"
      />
    </UiCard>
...
const { data: page, pending } = await useAsyncData(
  () => `user.${id.value}.following.${currentPage.value}`,
  () => useApi<UserListPage>(
    `/api/users/${id.value}/following?page=${currentPage.value}&size=${pageSize}`,
...
useHead(() => ({ title: user.value ? `Following · ${user.value.username}` : 'Following' }))
</script>
